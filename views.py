...
def welcome(request):
    context = {
        ...
        "myjobs" : Job.objects.filter(worker=User.objects.get(id=request.session['id']))
    }
...
def execute_dojob(request):
    if request.method == "POST":
        ...
        print("my worker jobs = ",Job.objects.filter(worker=User.objects.get(id=request.session['id'])))
